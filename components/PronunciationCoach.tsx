...
export const PronunciationCoach: React.FC<PronunciationCoachProps> = ({ onLogout }) => {
  ...
  useEffect(() => {
    // Auto-select first item when entering a practice view
  ...
  const handleSelectLevel = (level: PracticeLevel) => {
    ...
    setView('practice');
    // Reset sub-level states
    setSelectedItem(null); 
    setSelectedPhonemeCategory(null);
    setScoreResult(null);
    setUserAudio(null);
    setError(null);
  };
  ...
  const renderPracticeItems = () => {
    ...
    return (
      <div className="flex flex-wrap gap-2">
        {(data as PracticeItem[]).map((item, index) => (
          <button
            key={index}
            onClick={() => handleSelectItem(item)}
  ...
  const getPracticeTitle = () => {
    if (selectedLevel === PracticeLevel.Phonemes) {
      return selectedPhonemeCategory || '音标 (Phonemes)';
    }
    const levelMap = {
      [PracticeLevel.Words]: '单词 (Words)',
      [PracticeLevel.Phrases]: '短语 (Phrases)',
      [PracticeLevel.Sentences]: '句子 (Sentences)'
    };
  ...
  const renderPracticeContent = () => {
    if (!selectedLevel) {
      return <div>Error: No level selected.</div>;
    }
      ...
      <>
        <section id="practice-selection" className="mb-8">
          <div className="flex items-center justify-between mb-4">
              ...
              <button
                  onClick={handleBack}
                  className="px-4 py-2 text-sm font-medium text-orange-600 dark:text-orange-400 border border-orange-600 dark:border-orange-400 rounded-md hover:bg-orange-50 dark:hover:bg-gray-700 transition-colors"
              >
